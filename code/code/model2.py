...
def Model2 (input_shape = (96, 96, 1), classes = 2):
    ...
    X = Conv2D(filters=32,kernel_size=(3,3),strides=(1,1),padding ='valid')(X_input)
...
def create_point(name):
    ...
    fin = open("./10afterface/"+names[0]+".txt", 'r')
    x,y=fin.read().split(',')
...
model = Model2((96,96,1),2)
model.summary()

history = model.fit(X, y, epochs=500, validation_data=(X, y),
